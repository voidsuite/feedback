...
// Optional-session read: no session or a dead session is `{ user: null }`
// (200), not a 401, so signed-out clients don't trip the refresh-on-401
// retry and spam the console on every boot.
auth.get("/me", async (c) => {
  const sessionId = getCookie(c, getCookieName())
