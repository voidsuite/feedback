...
import { ChevronLeft, Cloud, FileText, Lock, ShieldCheck } from "lucide-react"
import { Button } from "@/components/ui/button"
import { DocsLogo } from "@/components/DocsLogo"
import { useAuth } from "@/contexts/auth"

/**
 * Login — VoidAuth-style landing for docs. Two steps: landing → confirm
 * remember. Offline builds (VITE_AUTH_MODE=offline) skip OAuth entirely.
 */
    ...
    <div className="flex min-h-screen flex-col items-center justify-center px-4">
      <div className="w-full max-w-sm space-y-8 text-center">
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">Encrypted, local-first collaborative documents</p>
        </div>
        ...
        <div className="space-y-3 rounded-xl border border-border bg-card/50 p-5 text-left">
          <div className="flex items-center gap-2.5">
            ...
            <p className="text-xs text-muted-foreground">Real-time collaboration via share links</p>
          </div>
          <div className="flex items-center gap-2.5">
