...
/** docs logo — VoidAuth-style ring + dot, next to the wordmark. */
export function DocsLogo({ size = "md", className, tagline = false }: DocsLogoProps) {
  ...
  return (
    <span className={cn("inline-flex items-center font-semibold tracking-tight", s.gap, className)}>
      ...
      <span className={s.text}>
        docs
        {tagline && (
          <span className="ml-1.5 hidden text-xs font-normal text-muted-foreground sm:inline">— Void Docs</span>
        )}
      </span>
    </span>
  )
