...
export function VerifyEmailPage() {
  ...
  useEffect(() => {
    ...
    if (status === 'loading') {
      confirmEmailVerification(token).then((result: any) => {
        if (result.error) {
          setStatus('error')
      ...
      <div className="w-full max-w-xs space-y-8 text-center">
        ...
        {status === 'loading' && (
          <div className="space-y-3 animate-in fade-in duration-200">
            <div className="mx-auto size-8 rounded-full border-2 border-border border-t-primary animate-spin" />
        ...
        {status === 'success' && (
          <div className="space-y-3 animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-xs text-muted-foreground">Your email has been confirmed. Redirecting...</p>
          </div>
        )}
