/**
 * Browser crypto helpers — same scheme as the VoidAuth ecosystem:
 * AES-GCM-256 with a key derived via PBKDF2 (SHA-256, 100k iterations).
 * Everything is encrypted client-side; nothing sensitive leaves the device
 * in plaintext.
 */
...
async function deriveKey(passphrase: string, salt: Uint8Array<ArrayBuffer>): Promise<CryptoKey> {
  const baseKey = await crypto.subtle.importKey("raw", getEncoder().encode(passphrase), KEY_DERIVE_ALGO, false, [
    "deriveKey",
  ])
...
export async function encrypt(data: string, passphrase: string): Promise<string> {
  ...
  const ciphertext = await crypto.subtle.encrypt({ name: ENC_ALGO, iv }, key, getEncoder().encode(data))
...
export async function encryptBytes(data: Uint8Array<ArrayBuffer>, passphrase: string): Promise<Uint8Array<ArrayBuffer>> {
  const salt = crypto.getRandomValues(new Uint8Array(SALT_LENGTH))
  const iv = crypto.getRandomValues(new Uint8Array(IV_LENGTH))
  const key = await deriveKey(passphrase, salt)
  const ciphertext = await crypto.subtle.encrypt({ name: ENC_ALGO, iv }, key, data)
  const combined = new Uint8Array(salt.length + iv.length + ciphertext.byteLength)
  combined.set(salt, 0)
  combined.set(iv, salt.length)
  combined.set(new Uint8Array(ciphertext), salt.length + iv.length)
...
export async function decryptBytes(encrypted: Uint8Array<ArrayBuffer>, passphrase: string): Promise<Uint8Array<ArrayBuffer>> {
...
// --- Device key (local-only encryption, never synced) ---

const DEVICE_KEY_NAME = "m3il_device_key"
...
export async function encryptWithDeviceKey(data: string): Promise<string> {
  ...
  const iv = crypto.getRandomValues(new Uint8Array(IV_LENGTH))
  const ciphertext = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, key, getEncoder().encode(data))
...
// --- Passphrase hashing (PIN verifier) ---

export async function hashPassphrase(passphrase: string, salt: Uint8Array<ArrayBuffer>): Promise<string> {
  const baseKey = await crypto.subtle.importKey("raw", getEncoder().encode(passphrase), KEY_DERIVE_ALGO, false, [
    "deriveBits",
  ])
